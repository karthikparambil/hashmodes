...
# Function to search for hash mode
search_mode() {
    local search_term="$1"
    local found=0
    ...
    # Process data from __DATA__ section
    # Using a temporary file or process substitution to avoid subshell issues with variable scope if possible,
    # but for simple 'found' flag, we can just use a return code or verify output.
    # However, to format the output nicely, we will iterate over matches.

    # We use grep to find matches first to check existence, then format.
    # Actually, let's iterate through Matches.
    
    # Read the data section, filter by search term, and format output
    while IFS='|' read -r mode_id description; do
        if [ -n "$mode_id" ]; then
            # Trim whitespace
            mode_id=$(echo "$mode_id" | xargs)
            description=$(echo "$description" | xargs)
            
            echo -e "${GREEN}Mode: ${mode_id}${NC}"
            echo -e "Name: ${description}"
            echo ""
            found=1
        fi
    done < <(sed '1,/^__DATA__$/d' "${BASH_SOURCE[0]}" | grep -i "$search_term")
...
if [ $# -eq 0 ]; then
    # Interactive mode
    while true; do
        echo -e "${YELLOW}Enter search term (or 'q' to exit):${NC}"
        read -p "> " term
        
        if [[ "${term,,}" == "q" ]] || [[ "${term,,}" == "exit" ]]; then
...
6700 | AIX {ssha1}
6800 | LastPass + LastPass sniffed 4
...
7701 | SAP CODVN B (BCODE) from RFC _READ_TABLE
7800 | SAP CODVN F/G (PASSCODE)
7801 | SAP CODVN F/G (PASSCODE) from RFC _READ_TABLE
7900 | Drupal7
8000 | Sybase ASE
...
26610 | MetaMask Wallet (short hash, plaintext check) 8 *
...
29700 | KeePass 1 (AES/Twofish) and KeePass 2 (AES) - keyfile only mode 32
...
30901 | Bitcoin raw private key (P2PKH), compressed 33 *
30902 | Bitcoin raw private key (P2PKH), uncompressed 34 *
30903 | Bitcoin raw private key (P2WPKH, Bech32), compressed 35 *
30904 | Bitcoin raw private key (P2WPKH, Bech32), uncompressed 36 *
30905 | Bitcoin raw private key (P2SH(P2WPKH)), compressed 37 *
30906 | Bitcoin raw private key (P2SH(P2WPKH)), uncompressed 38 *
31000 | BLAKE2s-256 *
31100 | SM3 *
31200 | Veeam VBK *
31300 | MS SNTP *
